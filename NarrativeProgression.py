...
calculates the length of each segment of text measured in characters. The results
...
sections_BNW = ["Dystopian World State", "Pre-dystopian reservation",
                "Dystopian World State", "Pre-dystopian reservation",
...
def find_transits_in_BNW(text):
    ...
    chunks = []
    for i in range(len(starts)):  # len(starts) = 5 --> range 0-4
        # if last chunk, calculate chunk length based on
        # chunk = length of text - start of last chunk --> i ==  4
        if (i == len(starts) - 1):
            #print(i)
...
def create_multi_row_diagram(title, starts, chunks, sections):
    """
    This function was improved with Microsoft Copilot.
    ...
    # x-axis label
    ax.set_xlabel("Narrative progression measured in characters")
...
def create_one_row_diagram(title, starts, chunks, sections):
    ...
    palette = {
        ...
        "Pre-dystopian reservation": "darkgreen"
    }
